          ...
          {users.map((user) => (
            <tr key={user.userID}>
              ...
              <td className="py-2 px-4 border border-gray-300 flex flex-wrap gap-3">
                ...
                <button 
                  style={{display: (user.status === 1 && user.pass && user.status2 == 0) ? 'inline-block' : 'none' , height:"50%"}}
                ...
                <button
                  style={{display: (user.status === 2 && user.auth && user.status2 == 0) ? 'inline-block' : 'none' , height:"50%"}}
                  className="w-[100px] btn btn-sm mb-2 btn-warning"
